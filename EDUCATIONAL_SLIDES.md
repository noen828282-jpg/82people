...
**큰 질문:** "왜 캐릭터한테 이름 (코다리·이태원·홍병길) 붙였나?"
...
```
이름:    한국 친근감 (코다리·이태원·미향·루나)
이모지:  💻 🧠 🎵 📺 — 한 글자에 정체성
페르소나: 시스템 프롬프트에 톤 정의
         (예: 미향 = 정중·친근, 홍병길 = 데이터 중심·솔직)
응답 패턴: "사장님" 호칭 일관
```
...
this._broadcastCorporate({
  ...
  brief: '유튜브 + 매출 종합',
  tasks: [
    {agent: 'youtube', emoji: '📺', name: '홍병길', task: '...'},
    {agent: 'business', emoji: '💼', name: '이태원', task: '...'}
  ]
});

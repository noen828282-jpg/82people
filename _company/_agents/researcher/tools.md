...
_하덕영 에이전트가 어떤 도구를 어디까지 자율적으로 쓸 수 있는지 정의합니다._
...
### 🔍 `web_search`
DuckDuckGo를 통해 웹에서 관련 정보 및 트렌드를 실시간으로 검색합니다.

- **실행 커맨드**: `python _company/_agents/researcher/tools/web_search.py "<검색어>"`
- **출력**: 표준 출력으로 마크다운 형식의 검색 결과 제공.

### 📖 `page_fetcher`
특정 URL의 웹페이지에서 HTML 태그를 제거하고 순수 텍스트 본문만 추출합니다.

- **실행 커맨드**: `python _company/_agents/researcher/tools/page_fetcher.py "<URL>"`
- **출력**: 표준 출력으로 페이지 텍스트 본문 제공 (최대 8,000자).

...
## 절대 금지
- 랜딩페이지 마크업(HTML/CSS/JS) 코딩 직접 구현 (사용자가 클로드에게 시킨다고 명시하였으므로 백엔드/API 연동과 계획 수립에만 집중할 것).
...
## 이번 주 목표
- PDF PrivateShield 프로젝트 배포를 위한 Vercel/Netlify 호스팅 정식 배포 링크 생성 및 활성화
- PayPal SDK의 Sandbox -> Live 결제 모드 전환 설정
- $1 실결제 검증 테스트 계획 수립 및 모니터링 모듈 구축
- SendGrid 등 이메일 자동 발송 API 연동 구현 완료
